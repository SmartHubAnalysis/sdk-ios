...
- (BOOL)sendata_clickableForRNView {
    // RN 可点击元素的区分
...
- (NSDictionary *)sendata_RNElementScreenProperties {
...
- (BOOL)sendata_isAutoTrackAppClick {
    // 判断是否被覆盖
    ...
    if ([self sendata_clickableForRNView]) {
        return YES;
    }

    // Native 全埋点被忽略元素，不可圈选，RN 全埋点事件由插件触发，不经过此判断
    if (self.sendata_isIgnored) {
        return NO;
    }
...
- (NSString *)sendata_similarPath {
    // 是否支持限定元素位置功能
    BOOL enableSupportSimilarPath = [NSStringFromClass(self.class) isEqualToString:@"UITabBarButton"];
    if (self.sendata_elementPosition && enableSupportSimilarPath) {
        NSString *similarPath = [NSString stringWithFormat:@"%@[-]",NSStringFromClass(self.class)];
        return similarPath;
    } else {
        return self.sendata_itemPath;
    }
}

#pragma mark SAVisualizedViewPathProperty
// 当前元素，前端是否渲染成可交互
- (BOOL)sendata_enableAppClick {
    // 是否在屏幕显示
    // 是否触发 $AppClick 事件
    return self.sendata_isVisible && self.sendata_isAutoTrackAppClick;
}

- (NSString *)sendata_elementValidContent {
    /*
    ...
    return self.sendata_elementContent;
}

/// 元素子视图
- (NSArray *)sendata_subElements {
    //  部分元素，忽略子视图
    ...
    if ([NSStringFromClass(self.class) isEqualToString:@"UILayoutContainerView"]) {
        if ([[self nextResponder] isKindOfClass:UIViewController.class]) {
            UIViewController *controller = (UIViewController *)[self nextResponder];
            return controller.sendata_subElements;
        }
    }

    NSMutableArray *newSubViews = [NSMutableArray array];
    for (UIView *view in self.subviews) {
        if (view.sendata_isVisible) {
...
- (NSString *)sendata_elementPath {
    // 处理特殊控件
    // UISegmentedControl 嵌套 UISegment 作为选项单元格，特殊处理
    if ([NSStringFromClass(self.class) isEqualToString:@"UISegment"]) {
        UISegmentedControl *segmentedControl = (UISegmentedControl *)[self superview];
        if ([segmentedControl isKindOfClass:UISegmentedControl.class]) {
            return [SAVisualizedUtils viewSimilarPathForView:segmentedControl atViewController:segmentedControl.sendata_viewController shouldSimilarPath:YES];
        }
    }
    // 支持自定义属性，可见元素均上传 elementPath
    return [SAVisualizedUtils viewSimilarPathForView:self atViewController:self.sendata_viewController shouldSimilarPath:YES];
}

- (NSString *)sendata_elementSelector {
    // 处理特殊控件
    // UISegmentedControl 嵌套 UISegment 作为选项单元格，特殊处理
    if ([NSStringFromClass(self.class) isEqualToString:@"UISegment"]) {
        UISegmentedControl *segmentedControl = (UISegmentedControl *)[self superview];
        if ([segmentedControl isKindOfClass:UISegmentedControl.class]) {
             ...
             */
            NSString *elementSelector = [SAVisualizedUtils viewPathForView:segmentedControl atViewController:segmentedControl.sendata_viewController];
            // 解析 UISegment 的显示序号 index
            NSString *postion = [self sendata_elementPosition];
            // 原始路径分割后的集合
    ...
    if (self.sendata_enableAppClick) {
        return [SAVisualizedUtils viewPathForView:self atViewController:self.sendata_viewController];
    } else {
        return nil;
    }
}

- (BOOL)sendata_isFromWeb {
    return NO;
}

- (BOOL)sendata_isListView {
    // UISegmentedControl 嵌套 UISegment 作为选项单元格，特殊处理
...
- (NSString *)sendata_screenName {
    ...
    if ([SAAutoTrackUtils isKindOfRNView:self]) {
        NSDictionary *screenProperties = [self sendata_RNElementScreenProperties];
    ...
    // 解析 Native 元素页面信息
    if (self.sendata_viewController) {
        NSDictionary *autoTrackScreenProperties = [SAAutoTrackUtils propertiesWithViewController:self.sendata_viewController];
...
- (CGRect)sendata_visibleFrame {
    ...
    if (self.superview) {
         ...
         */
        CGRect superViewValidFrame = [self.superview sendata_visibleFrame];
...
- (NSArray *)sendata_subElements {
    ...
    return [super sendata_subElements];
}
...
- (NSArray *)sendata_subElements {
    ...
    for (UIView *view in subviews) {
        if (view != self.rootViewController.view && view.sendata_isVisible) {
            /*
...
- (NSString *)sendata_itemPath {
    ...
    return [NSString stringWithFormat:@"%@/%@", super.sendata_itemPath, subPath];
}

- (NSString *)sendata_similarPath {
    return [NSString stringWithFormat:@"%@/UISegment[-]", super.sendata_itemPath];
}

- (NSString *)sendata_heatMapPath {
    NSString *subPath = [NSString stringWithFormat:@"UISegment[%ld]", (long)self.selectedSegmentIndex];
    return [NSString stringWithFormat:@"%@/%@", super.sendata_heatMapPath, subPath];
}
...
- (NSArray *)sendata_subElements {
    ...
    for (UIView *view in subviews) {
        if ([view isKindOfClass:UITableViewCell.class]) {
            if ([visibleCells containsObject:view] && view.sendata_isVisible) {
                [newSubviews addObject:view];
            }
        } else if (view.sendata_isVisible) {
            [newSubviews addObject:view];
        }
    }
    return newSubviews;
...
- (NSString *)sendata_heatMapPath {
    ...
    while (![currentTableView isKindOfClass:UITableView.class]) {
        ...
        if (!currentTableView) {
            return super.sendata_heatMapPath;
        }
    }
...
- (NSString *)sendata_itemPath {
    if (self.sendata_IndexPath) {
        return [self sendata_itemPathWithIndexPath:self.sendata_IndexPath];
    }
    return [super sendata_itemPath];
}
...
- (NSString *)sendata_similarPath {
    if (self.sendata_IndexPath) {
        return [self sendata_similarPathWithIndexPath:self.sendata_IndexPath];
    }
    return self.sendata_itemPath;
}
...
- (NSString *)sendata_itemPathWithIndexPath:(NSIndexPath *)indexPath {
    return [NSString stringWithFormat:@"%@[%ld][%ld]", NSStringFromClass(self.class), (long)indexPath.section, (long)indexPath.row];
}

- (NSString *)sendata_similarPathWithIndexPath:(NSIndexPath *)indexPath {
...
- (NSString *)sendata_elementPosition {
    if (self.sendata_IndexPath) {
        return [[NSString alloc] initWithFormat:@"%ld:%ld", (long)self.sendata_IndexPath.section, (long)self.sendata_IndexPath.row];
    }
    return nil;
}

- (BOOL)sendata_isListView {
    return self.sendata_elementPosition != nil;
}
@end
...
- (NSString *)sendata_similarPath {
    if (self.sendata_IndexPath) {
        ...
        return super.sendata_similarPath;
    }
}
...
- (NSString *)sendata_similarPathWithIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!elementInfo.isSupportElementPosition) {
        return [self sendata_itemPathWithIndexPath:indexPath];
    }
    return [NSString stringWithFormat:@"%@[%ld][-]", NSStringFromClass(self.class), (long)indexPath.section];
}
...
- (NSString *)sendata_elementPosition {
    ...
    if (self.sendata_IndexPath) {
        return [[NSString alloc] initWithFormat:@"%ld:%ld", (long)self.sendata_IndexPath.section, (long)self.sendata_IndexPath.item];
    }
    return nil;
}
