...
- (NSArray *)sendata_subElements {
        ...
        [childViewControllers enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(__kindof UIViewController *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            if (obj.isViewLoaded) {
                ...
                if ([subElements containsObject:objSuperview]) {
                    ...
                    if (objSuperview.sendata_isVisible && !isContainFullScreen) {
               ...
               // 正在全屏显示
                if (isFullScreenShow && obj.view.sendata_isVisible) {
    ...
    if (currentView && self.isViewLoaded && currentView.sendata_isVisible) {
        [subElements addObject:currentView];
    }
    return subElements;
}

- (void)sendata_visualize_viewDidAppear:(BOOL)animated {
    [self sendata_visualize_viewDidAppear:animated];
    ...
    if (SAAutoTrackManager.sharedInstance.configOptions.enableAutoTrackChildViewScreen ||
        ...
        [self sendata_readyEnterViewController];
    }
...
- (void)sendata_readyEnterViewController {
...
- (NSArray *)sendata_subElements {
    ...
    for (UIView *view in subViews) {
        if ([view isKindOfClass:UITabBar.class]) {
            // UITabBar 元素
            if (self.isViewLoaded && self.tabBar.sendata_isVisible) {
...
- (NSArray *)sendata_subElements {
    ...
    for (UIView *view in subViews) {
        if ([view isKindOfClass:UINavigationBar.class]) {
            // UINavigationBar 元素
            if (self.isViewLoaded && self.navigationBar.sendata_isVisible) {
