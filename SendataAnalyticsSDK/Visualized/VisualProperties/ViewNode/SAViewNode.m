...
- (void)buildNodeRelation {
    ...
    for (UIView *subView in view.subviews) {
        if (subView.sendata_viewNode) {
            // 可能存在其他父节点，进行移除
            if (subView.sendata_viewNode.nextNode && subView.sendata_viewNode.nextNode != self) {
                [subView.sendata_viewNode removeOldNodeRelation];
            }
            [subView.sendata_viewNode buildNextNodeRelationWithNextNode:self];
        }
    }
    ...
    SAViewNode *nextNode = nextView.sendata_viewNode;
    if (nextNode) {
...
- (void)refreshScreenName {
    [SACommonUtility performBlockOnMainThread:^{
        self.screenName = [self.view sendata_screenName];
    }];
}
...
+ (void)bindScreenNameWithClickableView:(UIView *)rnView {
    [rnView sendata_clickableForRNView];
}
