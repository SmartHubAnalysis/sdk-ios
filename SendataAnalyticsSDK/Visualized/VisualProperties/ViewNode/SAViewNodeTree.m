...
- (void)setupListeners {
    ...
    dispatch_once(&onceToken, ^{
         ...
         */
        NSError *error = nil;
        [UIView sa_swizzleMethod:@selector(didMoveToSuperview) withMethod:@selector(sendata_visualize_didMoveToSuperview) error:&error];
        if (error) {
        ...
        [UIView sa_swizzleMethod:@selector(didMoveToWindow) withMethod:@selector(sendata_visualize_didMoveToWindow) error:NULL];
        
        // 测试发现 UIAlertController.view 即 _UIAlertControllerView 显示，未执行 didMoveToWindow 和 didMoveToSuperview，但是其父视图调用了 didAddSubview
        [UIView sa_swizzleMethod:@selector(didAddSubview:) withMethod:@selector(sendata_visualize_didAddSubview:) error:NULL];
        
        // 兼容在业务中，动态修改了 keyWindow
        [UIWindow sa_swizzleMethod:@selector(becomeKeyWindow) withMethod:@selector(sendata_visualize_becomeKeyWindow) error:NULL];
        
        // 针对 tab 元素，调用 setSelectedItem 切换页面后，更新子视图页面名称
        [UITabBar sa_swizzleMethod:@selector(setSelectedItem:) withMethod:@selector(sendata_visualize_setSelectedItem:) error:NULL];
...
#pragma clang diagnostic pop
        ...
        [UIView sa_swizzleMethod:@selector(bringSubviewToFront:) withMethod:@selector(sendata_visualize_bringSubviewToFront:) error:NULL];
        
        [UIView sa_swizzleMethod:@selector(sendSubviewToBack:) withMethod:@selector(sendata_visualize_sendSubviewToBack:) error:NULL];
        
        // cell 被重用，需要重新计算 indexPath
        [UITableViewCell sa_swizzleMethod:@selector(prepareForReuse) withMethod:@selector(sendata_visualize_prepareForReuse) error:NULL];
        
        [UICollectionViewCell sa_swizzleMethod:@selector(prepareForReuse) withMethod:@selector(sendata_visualize_prepareForReuse) error:NULL];
        
        // HeaderFooterView 被重用，重新计算 index
        [UITableViewHeaderFooterView sa_swizzleMethod:@selector(prepareForReuse) withMethod:@selector(sendata_visualize_prepareForReuse) error:NULL];
    });
}
...
- (void)removeViewNodeWithView:(UIView *)view {
    if (view.superview || !view.sendata_viewNode) {
        return;
    }
...
- (void)addViewNodeWithView:(UIView *)view {
    ...
    if (view.sendata_viewNode) {
        SAViewNode *viewNode = view.sendata_viewNode;
        // 过滤重复构建
    ...
    for (SAViewNode *subNode in nextView.sendata_viewNode.subNodes) {
...
- (void)updateBrotherViewNodeWithView:(UIView *)view isAddViewNode:(BOOL)isAdd {
    SAViewNode *currentNode = view.sendata_viewNode;
...
- (void)refreshRNViewScreenNameWithViewController:(UIViewController *)viewController {
     ...
     */
    dispatch_async(dispatch_get_main_queue(), ^{
        SAViewNode *viewNode = viewController.view.sendata_viewNode;
...
- (void)refreshRNViewScreenName:(NSString *)screenName viewNode:(SAViewNode *)viewNode {
    if ([viewNode isKindOfClass:SARNViewNode.class] && [viewNode.view sendata_clickableForRNView] && ![screenName isEqualToString:viewNode.screenName]) {
        [viewNode refreshSubNodeScreenName];
        return;
    }
...
- (void)dealloc {
    ...
    for (SAViewNode *node in self.rootNode.subNodes) {
        node.view.sendata_viewNode = nil;
    }
}
