...
- (NSURLRequest *)buildConfigRequest {
    ...
    NSURLComponents *components = SendataAnalyticsSDK.sharedInstance.network.baseURLComponents;
    if (!components) {
        NSString *logMessage = [SAVisualizedLogger buildLoggerMessageWithTitle:@"获取配置" message:@"数据接收地址无效，serverURL: %@", SendataAnalyticsSDK.sharedInstance.network.serverURL];
    ...
    params[@"project"] = SendataAnalyticsSDK.sharedInstance.network.project;
...
- (void)unarchiveConfig {
    NSString *project = SendataAnalyticsSDK.sharedInstance.network.project;
...
- (nullable NSArray <SAVisualPropertiesConfig *> *)propertiesConfigsWithViewNode:(SAViewNode *)viewNode {
    ...
    for (SAVisualPropertiesConfig *config in configSources) {
        ...
        // 命中配置信息
        if (config.eventType == SendataAnalyticsEventTypeAppClick && [config.event isMatchVisualEventWithViewIdentify:viewNode]) {
...
- (nullable NSArray <SAVisualPropertiesConfig *> *)propertiesConfigsWithEventIdentifier:(SAEventIdentifier *)eventIdentifier {
    ...
    NSArray<SAVisualPropertiesConfig *> *configSources = self.configResponse.events;
    if (configSources.count == 0 || !eventIdentifier || eventIdentifier.eventType != SendataAnalyticsEventTypeAppClick) {
        return nil;
    }
    ...
    for (SAVisualPropertiesConfig *config in configSources) {
        // 命中 AppClick 配置
        if (config.eventType == SendataAnalyticsEventTypeAppClick && [config.event isMatchVisualEventWithViewIdentify:eventIdentifier]) {
            [configs addObject:config];
        }
    }
