...
#import "SendataAnalyticsSDK+Visualized.h"
...
/// 当前类型
@property (nonatomic, assign) SendataAnalyticsVisualizedType visualizedType;
...
- (void)configChangedWithValid:(BOOL)valid {
    if (valid){
        ...
        // 配置动态变化，开启埋点校验
        if (!self.eventCheck && self.visualizedType ==   SendataAnalyticsVisualizedTypeAutoTrack) {
...
- (void)setEnable:(BOOL)enable {
    ...
    dispatch_once(&onceToken, ^{
        NSError *error = nil;
        [UIViewController sa_swizzleMethod:@selector(viewDidAppear:) withMethod:@selector(sendata_visualize_viewDidAppear:) error:&error];
        if (error) {
...
- (NSString *)javaScriptSource {
    ...
    [javaScriptSource appendString:@"window.SenData_App_Visual_Bridge = {};"];
    if (self.visualizedConnection.isVisualizedConnecting) {
        [javaScriptSource appendFormat:@"window.SenData_App_Visual_Bridge.sendata_visualized_mode = true;"];
    }
...
- (BOOL)handleURL:(NSURL *)url {
    ...
    BOOL isEqualProject = [[SendataAnalyticsSDK sharedInstance].network.project isEqualToString:project];
    if (!isEqualProject) {
    ...
    // 未开启点击图
    if ([url.host isEqualToString:@"heatmap"] && ![[SendataAnalyticsSDK sharedInstance] isHeatMapEnabled]) {
        [SAVisualizedManager showAlterViewWithTitle:@"提示" message:@"SDK 没有被正确集成，请联系贵方技术人员开启点击分析"];
        return YES;
    }

    // 未开启可视化全埋点
    if ([url.host isEqualToString:@"visualized"] && ![[SendataAnalyticsSDK sharedInstance] isVisualizedAutoTrackEnabled]) {
...
- (void)showOpenAlertWithURL:(NSURL *)URL featureCode:(NSString *)featureCode postURL:(NSString *)postURL {
    ...
    [alertController addActionWithTitle:@"继续" style:SAAlertActionStyleDefault handler:^(SAAlertAction *_Nonnull action) {
        ...
        if ([self isHeatMapURL:URL]) {
            ...
            self.visualizedType = SendataAnalyticsVisualizedTypeHeatMap;
        } else if ([self isVisualizedAutoTrackURL:URL]) {
            SALogDebug(@"Confirmed to open VisualizedAutoTrack ...");
            self.visualizedType =   SendataAnalyticsVisualizedTypeAutoTrack;
...
/// 当前类型
- (SendataAnalyticsVisualizedType)currentVisualizedType {
...
- (nullable NSDictionary *)propertiesWithView:(UIView *)view {
    UIViewController<SAAutoTrackViewControllerProperty> *viewController = view.sendata_viewController;
    if (!viewController) {
        return nil;
    }
