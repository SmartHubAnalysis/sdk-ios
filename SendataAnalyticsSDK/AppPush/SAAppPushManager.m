...
- (void)setConfigOptions:(SAConfigOptions *)configOptions {
    ...
    [UIApplication sharedApplication].sendata_launchOptions = configOptions.launchOptions;
}
...
- (void)proxyNotifications {
    ...
    if (@available(iOS 10.0, *)) {
        ...
        NSError *error = NULL;
        [UNUserNotificationCenter sa_swizzleMethod:@selector(setDelegate:) withMethod:@selector(sendata_setDelegate:) error:&error];
        if (error) {
