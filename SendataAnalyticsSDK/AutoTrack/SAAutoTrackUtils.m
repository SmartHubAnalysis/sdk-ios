...
+ (BOOL)isValidAppClickForObject:(id<SAAutoTrackViewProperty>)object {
    ...
    if (![object respondsToSelector:@selector(sendata_timeIntervalForLastAppClicksendata_timeIntervalForLastAppClick)]) {
        return YES;
    }

    NSTimeInterval lastTime = object.sendata_timeIntervalForLastAppClick;
...
+ (NSDictionary<NSString *, NSString *> *)propertiesWithViewController:(UIViewController<SAAutoTrackViewControllerProperty> *)viewController {
    ...
    properties[kSAEventPropertyScreenName] = viewController.sendata_screenName;
    properties[kSAEventPropertyTitle] = viewController.sendata_title;
...
+ (NSMutableDictionary<NSString *, NSString *> *)propertiesWithAutoTrackObject:(id<SAAutoTrackViewProperty>)object viewController:(nullable UIViewController<SAAutoTrackViewControllerProperty> *)viewController isCodeTrack:(BOOL)isCodeTrack {
    if (![object respondsToSelector:@selector(sendata_isIgnoredsendata_isIgnored)] || (!isCodeTrack && object.sendata_isIgnored)) {
        return nil;
    }

    NSMutableDictionary *properties = [[NSMutableDictionary alloc] init];
    // ViewID
    properties[kSAEventPropertyElementId] = object.sendata_elementId;

    viewController = viewController ? : object.sendata_viewController;
    if (!isCodeTrack && viewController.sendata_isIgnored) {
        return nil;
    }
    ...
    properties[kSAEventPropertyElementType] = object.sendata_elementType;
    properties[kSAEventPropertyElementContent] = object.sendata_elementContent;
    properties[kSAEventPropertyElementPosition] = object.sendata_elementPosition;

    UIView *view = (UIView *)object;
    //View Properties
    if ([object isKindOfClass:UIView.class]) {
        [properties addEntriesFromDictionary:view.sendataAnalyticsViewProperties];
    } else {
        return properties;
    }
...
+ (NSMutableDictionary<NSString *, NSString *> *)propertiesWithAutoTrackObject:(UIScrollView<SAAutoTrackViewProperty> *)object didSelectedAtIndexPath:(NSIndexPath *)indexPath {
    if (![object respondsToSelector:@selector(sendata_isIgnoredsendata_isIgnored)] || object.sendata_isIgnored) {
        return nil;
    }
    NSMutableDictionary *properties = [[NSMutableDictionary alloc] init];
    ...
    UIViewController<SAAutoTrackViewControllerProperty> *viewController = object.sendata_viewController;
    if (viewController.sendata_isIgnored) {
        return nil;
    }
    ...
    properties[kSAEventPropertyElementType] = object.sendata_elementType;
    properties[kSAEventPropertyElementContent] = cell.sendata_elementContent;
    properties[kSAEventPropertyElementPosition] = [cell sendata_elementPositionWithIndexPath:indexPath];

    //View Properties
    NSDictionary *viewProperties = ((UIView *)object).sendataAnalyticsViewProperties;
...
+ (NSDictionary *)propertiesWithAutoTrackDelegate:(UIScrollView *)scrollView didSelectedAtIndexPath:(NSIndexPath *)indexPath {
    ...
    @try {
        if ([scrollView isKindOfClass:UITableView.class]) {
            ...
            if ([tableView.sendataAnalyticsDelegate respondsToSelector:@selector(sendataAnalytics_tableView:autoTrackPropertiesAtIndexPath:)]) {
                properties = [tableView.sendataAnalyticsDelegate sendataAnalytics_tableView:tableView autoTrackPropertiesAtIndexPath:indexPath];
            }
        } else if ([scrollView isKindOfClass:UICollectionView.class]) {
            UICollectionView *collectionView = (UICollectionView *)scrollView;
            if ([collectionView.sendataAnalyticsDelegate respondsToSelector:@selector(sendataAnalytics_collectionView:autoTrackPropertiesAtIndexPath:)]) {
                properties = [collectionView.sendataAnalyticsDelegate sendataAnalytics_collectionView:collectionView autoTrackPropertiesAtIndexPath:indexPath];
            }
        }
