...
- (BOOL)sendata_isIgnored {
    if (self.isHidden || self.sendataAnalyticsIgnoreView) {
        return YES;
    }
...
- (void)setSendata_timeIntervalForLastAppClick:(NSTimeInterval)sendata_timeIntervalForLastAppClick {
    objc_setAssociatedObject(self, kSALastAppClickIntervalPropertyName, [NSNumber numberWithDouble:sendata_timeIntervalForLastAppClick], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSTimeInterval)sendata_timeIntervalForLastAppClick {
    return [objc_getAssociatedObject(self, kSALastAppClickIntervalPropertyName) doubleValue];
}

- (NSString *)sendata_elementType {
    SAViewElementInfo *elementInfo = [SAViewElementInfoFactory elementInfoWithView:self];
    return elementInfo.elementType;
}

- (NSString *)sendata_elementContent {
...
#pragma clang diagnostic pop
    ...
    if ([[self nextResponder] isKindOfClass:UITextField.class] && ![self isKindOfClass:UIButton.class]) {
        ...
        return [textField sendata_elementContent];
    }
    ...
    for (UIView *subview in self.subviews) {
        // 忽略隐藏控件
        if (subview.isHidden || subview.sendataAnalyticsIgnoreView) {
            continue;
        }
        NSString *temp = subview.sendata_elementContent;
        if (temp.length > 0) {
...
- (NSString *)sendata_elementPosition {
    ...
    if (superview && superview.sendata_elementPosition) {
        return superview.sendata_elementPosition;
    }
    return nil;
}

- (NSString *)sendata_elementId {
    return self.sendataAnalyticsViewID;
}

- (UIViewController *)sendata_viewController {
...
- (NSString *)sendata_elementContent {
    return self.text ?: super.sendata_elementContent;
}
...
- (NSString *)sendata_elementContent {
    NSString *imageName = self.image.sendataAnalyticsImageName;
    if (imageName.length > 0) {
        return [NSString stringWithFormat:@"%@", imageName];
    }
    return super.sendata_elementContent;
}
...
- (NSString *)sendata_elementPosition {
    ...
    return [super sendata_elementPosition];
}
...
- (BOOL)sendata_isIgnored {
    // 忽略 UITabBarItem
    BOOL ignoredUITabBarItem = [[SendataAnalyticsSDK sdkInstance] isViewTypeIgnored:UITabBarItem.class] && [NSStringFromClass(self.class) isEqualToString:@"UITabBarButton"];

    // 忽略 UIBarButtonItem
    BOOL ignoredUIBarButtonItem = [[SendataAnalyticsSDK sdkInstance] isViewTypeIgnored:UIBarButtonItem.class] && ([NSStringFromClass(self.class) isEqualToString:@"UINavigationButton"] || [NSStringFromClass(self.class) isEqualToString:@"_UIButtonBarButton"]);

    return super.sendata_isIgnored || ignoredUITabBarItem || ignoredUIBarButtonItem;
}
...
- (NSString *)sendata_elementContent {
    ...
    if (!text) {
        text = super.sendata_elementContent;
    }
    return text;
}
...
- (BOOL)sendata_isIgnored {
    return super.sendata_isIgnored || self.selectedSegmentIndex == UISegmentedControlNoSegment;
}

- (NSString *)sendata_elementContent {
    return  self.selectedSegmentIndex == UISegmentedControlNoSegment ? [super sendata_elementContent] : [self titleForSegmentAtIndex:self.selectedSegmentIndex];
}

- (NSString *)sendata_elementPosition {
    return self.selectedSegmentIndex == UISegmentedControlNoSegment ? [super sendata_elementPosition] : [NSString stringWithFormat: @"%ld", (long)self.selectedSegmentIndex];
}
...
- (BOOL)sendata_isIgnored {
    return self.tracking || super.sendata_isIgnored;
}
...
- (NSString *)sendata_elementPositionWithIndexPath:(NSIndexPath *)indexPath {
