...
static NSArray <UIView *>* sendata_searchVisualSubView(NSString *type, UIView *view) {
    NSMutableArray *subViews = [NSMutableArray array];
    for (UIView *subView in view.subviews) {
        if ([type isEqualToString:NSStringFromClass(subView.class)]) {
            [subViews addObject:subView];
        } else {
            NSArray *array = sendata_searchVisualSubView(type, subView);
            if (array.count > 0) {
...
- (BOOL)isTrackable {
    ...
    if ([SAGestureTargetActionModel filterValidModelsFrom:self.gesture.sendata_targetActionModels].count == 0) {
        return NO;
    }
    return YES;
}
...
- (BOOL)isIgnoreWithView:(UIView *)view {
    ...
    dispatch_once(&onceToken, ^{
        NSBundle *sendataBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:self.class] pathForResource:@"SendataAnalyticsSDK" ofType:@"bundle"]];
        NSString *jsonPath = [sendataBundle pathForResource:@"sa_autotrack_gestureview_blacklist.json" ofType:nil];
...
- (UIView *)trackableView {
    NSArray <UIView *>*visualViews = sendata_searchVisualSubView(@"_UIAlertControllerCollectionViewCell", self.gesture.view);
...
- (UIView *)trackableView {
    NSArray <UIView *>*visualViews = sendata_searchVisualSubView(@"_UIInterfaceActionCustomViewRepresentationView", self.gesture.view);
    CGPoint currentPoint = [self.gesture locationInView:self.gesture.view];
    for (UIView *visualView in visualViews) {
        CGRect rect = [visualView convertRect:visualView.bounds toView:self.gesture.view];
        if (CGRectContainsPoint(rect, currentPoint)) {
...
- (UIView *)trackableView {
    NSArray <UIView *>*visualViews = sendata_searchVisualSubView(@"_UIContextMenuActionView", self.gesture.view);
    CGPoint currentPoint = [self.gesture locationInView:self.gesture.view];
    for (UIView *visualView in visualViews) {
        CGRect rect = [visualView convertRect:visualView.bounds toView:self.gesture.view];
        if (CGRectContainsPoint(rect, currentPoint)) {
...
- (UIView *)trackableView {
    NSArray <UIView *>*visualViews = sendata_searchVisualSubView(@"_UIContextMenuActionsListCell", self.gesture.view);
    CGPoint currentPoint = [self.gesture locationInView:self.gesture.view];
    for (UIView *visualView in visualViews) {
        CGRect rect = [visualView convertRect:visualView.bounds toView:self.gesture.view];
        if (CGRectContainsPoint(rect, currentPoint)) {
...
- (BOOL)isTrackable {
    ...
    for (SAGestureTargetActionModel *model in self.gesture.sendata_targetActionModels) {
