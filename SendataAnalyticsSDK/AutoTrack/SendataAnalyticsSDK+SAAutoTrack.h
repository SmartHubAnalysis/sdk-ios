...
//UITableView
@optional
- (NSDictionary *)sendataAnalytics_tableView:(UITableView *)tableView autoTrackPropertiesAtIndexPath:(NSIndexPath *)indexPath;

//UICollectionView
@optional
- (NSDictionary *)sendataAnalytics_collectionView:(UICollectionView *)collectionView autoTrackPropertiesAtIndexPath:(NSIndexPath *)indexPath;
@end

@interface UIImage (SendataAnalytics)
@property (nonatomic, copy) NSString* sendataAnalyticsImageName;
@end

@interface UIView (SendataAnalytics)
- (nullable UIViewController *)sendataAnalyticsViewController __attribute__((deprecated("已过时")));

/// viewID
@property (nonatomic, copy) NSString* sendataAnalyticsViewID;

/// AutoTrack 时，是否忽略该 View
@property (nonatomic, assign) BOOL sendataAnalyticsIgnoreView;

/// AutoTrack 发生在 SendAction 之前还是之后，默认是 SendAction 之前
@property (nonatomic, assign) BOOL sendataAnalyticsAutoTrackAfterSendAction;

/// AutoTrack 时，View 的扩展属性
@property (nonatomic, strong) NSDictionary* sendataAnalyticsViewProperties;

@property (nonatomic, weak, nullable) id<SAUIViewAutoTrackDelegate> sendataAnalyticsDelegate;
@end
...
@interface SendataAnalyticsSDK (SAAutoTrack)
 ...
 * 该功能默认关闭
 */
- (void)enableAutoTrack:(SendataAnalyticsAutoTrackEventType)eventType __attribute__((deprecated("已过时，请参考 SAConfigOptions 类的 autoTrackEventType")));
 ...
 */
- (void)ignoreAutoTrackEventType:(SendataAnalyticsAutoTrackEventType)eventType __attribute__((deprecated("已过时，请参考enableAutoTrack:(SendataAnalyticsAutoTrackEventType)eventType")));
