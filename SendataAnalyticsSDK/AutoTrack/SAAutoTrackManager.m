...
- (BOOL)isAutoTrackEnabled {
    ...
    if (autoTrackMode == kSAAutoTrackModeDefault) {
        ...
        return (self.configOptions.autoTrackEventType !=   SendataAnalyticsEventTypeNone);
    } else {
...
- (BOOL)isAutoTrackEventTypeIgnored:(SendataAnalyticsAutoTrackEventType)eventType {
    ...
    if (autoTrackMode == kSAAutoTrackModeDefault) {
        ...
        if (isIgnored) {
            ...
            switch (eventType) {
                case   SendataAnalyticsEventTypeAppStart:
                    ignoredEvent = kSAEventNameAppStart;
                    break;

                case   SendataAnalyticsEventTypeAppEnd:
                    ignoredEvent = kSAEventNameAppEnd;
                    break;

                case   SendataAnalyticsEventTypeAppClick:
                    ignoredEvent = kSAEventNameAppClick;
                    break;

                case   SendataAnalyticsEventTypeAppViewScreen:
...
- (void)updateAutoTrackEventType {
    self.appStartTracker.ignored = [self isAutoTrackEventTypeIgnored:  SendataAnalyticsEventTypeAppStart];
    self.appEndTracker.ignored = [self isAutoTrackEventTypeIgnored:  SendataAnalyticsEventTypeAppEnd];
    self.appViewScreenTracker.ignored = [self isAutoTrackEventTypeIgnored:  SendataAnalyticsEventTypeAppViewScreen];
    self.appClickTracker.ignored = [self isAutoTrackEventTypeIgnored:  SendataAnalyticsEventTypeAppClick];
}
...
- (BOOL)isGestureVisualView:(id)obj {
    ...
    for (UIGestureRecognizer *gesture in view.gestureRecognizers) {
        if (gesture.sendata_gestureTarget) {
...
- (void)enableAppClickAutoTrack {
    ...
    [UIGestureRecognizer sa_swizzleMethod:@selector(initWithTarget:action:)
                               withMethod:@selector(sendata_initWithTarget:action:)
                                    error:NULL];
    [UIGestureRecognizer sa_swizzleMethod:@selector(addTarget:action:)
                               withMethod:@selector(sendata_addTarget:action:)
                                    error:NULL];
    [UIGestureRecognizer sa_swizzleMethod:@selector(removeTarget:action:)
                               withMethod:@selector(sendata_removeTarget:action:)
                                    error:NULL];
