...
- (void)filterValidSourceChannnels:(NSArray *)sourceChannels {
    NSSet *reservedPropertyName = sendata_reserved_properties();
...
- (BOOL)checkDeepLinkMode:(NSURL *)url {
    ...
    if ([self isValidURLForServerMode:url]) {
        ...
        [SendataAnalyticsSDK.sharedInstance asyncTrackEventObject:object properties:properties];
...
- (BOOL)isValidURLForServerMode:(NSURL *)url {
    ...
    NSString *host = SendataAnalyticsSDK.sharedInstance.network.serverURL.host;
    return ([url.host isEqualToString:@"sendata"] || [url.host isEqualToString:host]);
}
...
- (void)trackDeeplinkMatchedResult:(NSURL *)url result:(NSDictionary *)result interval:(NSTimeInterval)interval errorMsg:(NSString *)errorMsg {
    ...
    [SendataAnalyticsSDK.sharedInstance asyncTrackEventObject:object properties:props];
}
