...
static NSString *const kSACAIDCacheKey = @"com.sendata.caid.cache";
...
+ (void)load {
    ...
    SEL swizzledSel = NSSelectorFromString(@"sendata_getCAIDAsyncly:");
    [CAID sa_swizzleMethod:origSel withClass:SACAIDUtils.class withMethod:swizzledSel error:nil];
}

- (void)sendata_getCAIDAsyncly:(void(^)(id error, id caidStruct))callback {
    [self sendata_getCAIDAsyncly:^(id _Nonnull error, id _Nonnull caidStruct) {
