...
- (void)trackAppRemoteConfigChanged:(NSDictionary<NSString *, id> *)remoteConfig {
    ...
    [SendataAnalyticsSDK.sdkInstance asyncTrackEventObject:object properties:@{kSAEventPropertyAppRemoteConfig : eventConfigString ?: @""}];
    // 触发 $AppRemoteConfigChanged 时 flush 一次
    [SendataAnalyticsSDK.sdkInstance flush];
}
...
- (BOOL)isLibVersionUnchanged {
    return [self.model.localLibVersion isEqualToString:SendataAnalyticsSDK.sdkInstance.libVersion];
}
