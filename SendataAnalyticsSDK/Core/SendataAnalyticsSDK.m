...
void *SendataAnalyticsQueueTag = &SendataAnalyticsQueueTag;

static dispatch_once_t sdkInitializeOnceToken;
static SendataAnalyticsSDK *sharedInstance = nil;

@interface SendataAnalyticsSDK()

// 在内部，重新声明成可读写的
@property (atomic, strong) SendataAnalyticsPeople *people;
...
@implementation SendataAnalyticsSDK {
    SendataAnalyticsNetworkType _networkTypePolicy;
}

#pragma mark - Initialization
+ (void)startWithConfigOptions:(SAConfigOptions *)configOptions {
    NSAssert(sendata_is_same_queue(dispatch_get_main_queue()), @"神策 iOS SDK 必须在主线程里进行初始化，否则会引发无法预料的问题（比如丢失 $AppStart 事件）。");
#if TARGET_OS_IOS
    ...
    dispatch_once(&sdkInitializeOnceToken, ^{
        sharedInstance = [[SendataAnalyticsSDK alloc] initWithConfigOptions:configOptions debugMode:  SendataAnalyticsDebugOff];
        [SAModuleManager startWithConfigOptions:sharedInstance.configOptions debugMode:  SendataAnalyticsDebugOff];
        [sharedInstance startAppLifecycle];
    });
}

+ (SendataAnalyticsSDK *_Nullable)sharedInstance {
...
+ (SendataAnalyticsSDK *)sdkInstance {
    NSAssert(sharedInstance, @"请先使用 startWithConfigOptions: 初始化 SDK");
    return sharedInstance;
}
...
- (instancetype)initWithServerURL:(NSString *)serverURL
                 andLaunchOptions:(NSDictionary *)launchOptions
                     andDebugMode:(SendataAnalyticsDebugMode)debugMode {
    @try {
...
- (instancetype)initWithConfigOptions:(nonnull SAConfigOptions *)configOptions debugMode:(SendataAnalyticsDebugMode)debugMode {
    @try {
        self = [super init];
        if (self) {
            ...
            _networkTypePolicy =
#if TARGET_OS_IOS
              SendataAnalyticsNetworkType3G |
              SendataAnalyticsNetworkType4G |
            
#ifdef __IPHONE_14_1
              SendataAnalyticsNetworkType5G |
#endif
#endif
              SendataAnalyticsNetworkTypeWIFI;
            
            _people = [[SendataAnalyticsPeople alloc] init];

            NSString *serialQueueLabel = [NSString stringWithFormat:@"com.sendata.serialQueue.%p", self];
            _serialQueue = dispatch_queue_create([serialQueueLabel UTF8String], DISPATCH_QUEUE_SERIAL);
            dispatch_queue_set_specific(_serialQueue, SendataAnalyticsQueueTag, &SendataAnalyticsQueueTag, NULL);

            NSString *readWriteQueueLabel = [NSString stringWithFormat:@"com.sendata.readWriteQueue.%p", self];
...
- (void)setFlushNetworkPolicy:(SendataAnalyticsNetworkType)networkType {
    @synchronized (self) {
...
- (SendataAnalyticsDebugMode)debugMode {
...
- (void)remoteConfigManagerModelChanged:(NSNotification *)sender {
    @try {
        ...
        if (isDisableDebugMode) {
            SAModuleManager.sharedInstance.debugMode =   SendataAnalyticsDebugOff;
        }
...
@implementation SendataAnalyticsSDK (Deeplink)
...
@implementation SendataAnalyticsSDK (JSCall)
...
@implementation SendataAnalyticsPeople

- (void)set:(NSDictionary *)profileDict {
    if (profileDict) {
        [[SendataAnalyticsSDK sharedInstance] profile:SA_PROFILE_SET properties:profileDict];
    }
}

- (void)setOnce:(NSDictionary *)profileDict {
    if (profileDict) {
        [[SendataAnalyticsSDK sharedInstance] profile:SA_PROFILE_SET_ONCE properties:profileDict];
    }
}

- (void)set:(NSString *) profile to:(id)content {
    if (profile && content) {
        [[SendataAnalyticsSDK sharedInstance] profile:SA_PROFILE_SET properties:@{profile: content}];
    }
}

- (void)setOnce:(NSString *) profile to:(id)content {
    if (profile && content) {
        [[SendataAnalyticsSDK sharedInstance] profile:SA_PROFILE_SET_ONCE properties:@{profile: content}];
    }
}

- (void)unset:(NSString *) profile {
    if (profile) {
        [[SendataAnalyticsSDK sharedInstance] profile:SA_PROFILE_UNSET properties:@{profile: @""}];
    }
}
...
- (void)increment:(NSString *)profile by:(NSNumber *)amount {
    if (profile && amount) {
        ...
        [SendataAnalyticsSDK.sharedInstance asyncTrackEventObject:object properties:@{profile: amount}];
    }
}
...
- (void)increment:(NSDictionary *)profileDict {
    if (profileDict) {
        SAProfileIncrementEventObject *object = [[SAProfileIncrementEventObject alloc] initWithType:SA_PROFILE_INCREMENT];
        [SendataAnalyticsSDK.sharedInstance asyncTrackEventObject:object properties:profileDict];
    }
}
...
- (void)append:(NSString *)profile by:(NSObject<NSFastEnumeration> *)content {
    if (profile && content) {
        if ([content isKindOfClass:[NSSet class]] || [content isKindOfClass:[NSArray class]]) {
            ...
            [SendataAnalyticsSDK.sharedInstance asyncTrackEventObject:object properties:@{profile: content}];
        }
    }
}

- (void)deleteUser {
    [[SendataAnalyticsSDK sharedInstance] profile:SA_PROFILE_DELETE properties:@{}];
}

@end

#pragma mark - Deprecated
@implementation SendataAnalyticsSDK (Deprecated)

+ (SendataAnalyticsSDK *)sharedInstanceWithConfig:(nonnull SAConfigOptions *)configOptions {
    [self startWithConfigOptions:configOptions];
    return sharedInstance;
}

+ (SendataAnalyticsSDK *)sharedInstanceWithServerURL:(NSString *)serverURL
                                        andDebugMode:(SendataAnalyticsDebugMode)debugMode {
    return [SendataAnalyticsSDK sharedInstanceWithServerURL:serverURL
...
+ (SendataAnalyticsSDK *)sharedInstanceWithServerURL:(nonnull NSString *)serverURL
                                    andLaunchOptions:(NSDictionary * _Nullable)launchOptions {
    NSAssert(sendata_is_same_queue(dispatch_get_main_queue()), @"神策 iOS SDK 必须在主线程里进行初始化，否则会引发无法预料的问题（比如丢失 $AppStart 事件）。");
    dispatch_once(&sdkInitializeOnceToken, ^{
        sharedInstance = [[self alloc] initWithServerURL:serverURL
                                        andLaunchOptions:launchOptions
                                            andDebugMode:  SendataAnalyticsDebugOff];
        [SAModuleManager startWithConfigOptions:sharedInstance.configOptions debugMode:  SendataAnalyticsDebugOff];
        [sharedInstance startAppLifecycle];
    });
    return sharedInstance;
...
- (void)setDebugMode:(SendataAnalyticsDebugMode)debugMode {
...
- (void)trackTimerBegin:(NSString *)event withTimeUnit:(SendataAnalyticsTimeUnit)timeUnit {
    UInt64 currentSysUpTime = [self.class getSystemUpTime];
    dispatch_async(self.serialQueue, ^{
...
- (void)trackTimer:(NSString *)event {
    [self trackTimer:event withTimeUnit:  SendataAnalyticsTimeUnitMilliseconds];
}

- (void)trackTimer:(NSString *)event withTimeUnit:(SendataAnalyticsTimeUnit)timeUnit {
    UInt64 currentSysUpTime = [self.class getSystemUpTime];
    dispatch_async(self.serialQueue, ^{
        [self.trackTimer trackTimerStart:event timeUnit:timeUnit currentSysUpTime:currentSysUpTime];
    });
