...
NSString * const SA_SCRIPT_MESSAGE_HANDLER_NAME = @"sendataNativeTracker";

NSSet* sendata_reserved_properties() {
...
BOOL sendata_is_same_queue(dispatch_queue_t queue) {
    return strcmp(dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL), dispatch_queue_get_label(queue)) == 0;
}

void sendata_dispatch_safe_sync(dispatch_queue_t queue,DISPATCH_NOESCAPE dispatch_block_t block) {
...
NSNotificationName const SA_TRACK_EVENT_NOTIFICATION = @"SendataAnalyticsTrackEventNotification";
NSNotificationName const SA_TRACK_LOGIN_NOTIFICATION = @"SendataAnalyticsTrackLoginNotification";
NSNotificationName const SA_TRACK_LOGOUT_NOTIFICATION = @"SendataAnalyticsTrackLogoutNotification";
NSNotificationName const SA_TRACK_IDENTIFY_NOTIFICATION = @"SendataAnalyticsTrackIdentifyNotification";
NSNotificationName const SA_TRACK_RESETANONYMOUSID_NOTIFICATION = @"SendataAnalyticsTrackResetAnonymousIdNotification";
NSNotificationName const SA_TRACK_EVENT_H5_NOTIFICATION = @"SendataAnalyticsTrackEventFromH5Notification";

#pragma mark - ABTest related notifications
NSNotificationName const SA_H5_BRIDGE_NOTIFICATION = @"SendataAnalyticsRegisterJavaScriptBridgeNotification";

NSNotificationName const SA_H5_MESSAGE_NOTIFICATION = @"SendataAnalyticsMessageFromH5Notification";

#pragma mark - other
// 远程配置更新
NSNotificationName const SA_REMOTE_CONFIG_MODEL_CHANGED_NOTIFICATION = @"cn.sendata.SA_REMOTE_CONFIG_MODEL_CHANGED_NOTIFICATION";

// App 内嵌 H5 接收可视化相关 H5 页面元素信息
NSNotificationName const SA_VISUALIZED_H5_MESSAGE_NOTIFICATION = @"SendataAnalyticsVisualizedMessageFromH5Notification";
