...
- (Class)class {
    if (self.sendata_delegateClass) {
        return self.sendata_delegateClass;
    }
    return [super class];
}
...
+ (void)proxyDelegate:(id)delegate selectors:(NSSet<NSString *> *)selectors {
    ...
    // 当前代理对象已经处理过
    if ([delegate sendata_superClass]) {
        NSMutableSet *currentSelectors = [NSMutableSet setWithSet:[delegate sendata_selectors]];
        ...
        [delegate setSendata_selectors:[delegateSelectors copy]];
        [delegate setSendata_delegateProxy:self];
        return;
    }
    ...
    if ([self isKVOClass:realClass]) {
        // KVO 的类, SDK 未新建子类
        [delegate setSendata_superClass:[SAClassHelper realSuperClassWithClass:realClass]];
        // 记录 - class 方法所返回的信息, 用于重写 - class 方法
        [delegate setSendata_delegateClass:[delegate class]];
    ...
    // 创建类
    NSString *dynamicClassName = [SADelegateProxy generateSendataClassName:delegate];
    ...
    [delegate setSendata_superClass:realClass];
    // 记录 - class 方法所返回的信息, 用于重写 - class 方法
    [delegate setSendata_delegateClass:[delegate class]];
    ...
    if ([SAClassHelper setObject:delegate toClass:dynamicClass]) {
        // 在对象释放时, 释放创建的子类
        [delegate sendata_registerDeallocBlock:^{
...
+ (void)invokeWithTarget:(NSObject *)target selector:(SEL)selector, ... {
    Class originalClass = target.sendata_superClass ?: target.superclass;
...
+ (void)resolveOptionalSelectorsForDelegate:(id)delegate {
    ...
    NSSet *currentOptionalSelectors = ((NSObject *)delegate).sendata_optionalSelectors;
    ...
    ((NSObject *)delegate).sendata_optionalSelectors = [optionalSelectors copy];
}
...
- (void)removeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath {
    ...
    if (oldClassIsKVO && !newClassIsKVO) {
        // 清空已经记录的原始类
        self.sendata_superClass = nil;
        Class delegateProxy = [self.sendata_delegateProxy class];
        if ([delegateProxy respondsToSelector:NSSelectorFromString(@"proxyDelegate:selectors:")]) {
            [delegateProxy proxyDelegate:self selectors:self.sendata_selectors];
        }
    }
}
...
#pragma mark - Utils
/// Delegate 的类前缀
static NSString *const kSADelegateSuffix = @"__CN.SENDATA";
...
/// 是不是神策创建的类
/// @param cls 类
+ (BOOL)isSendataClass:(Class _Nullable)cls {
    return [NSStringFromClass(cls) containsString:kSADelegateSuffix];
}

/// 生成神策要创建类的类名
/// @param obj 实例对象
+ (NSString *)generateSendataClassName:(id)obj {
    Class class = [SAClassHelper realClassWithObject:obj];
    if ([SADelegateProxy isSendataClass:class]) return NSStringFromClass(class);
