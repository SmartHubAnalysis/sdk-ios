...
- (NSData *)buildBodyWithJSONString:(NSString *)jsonString isEncrypted:(BOOL)isEncrypted {
    ...
    int hashCode = [jsonString sendata_hashCode];
...
- (NSURLRequest *)buildFlushRequestWithServerURL:(NSURL *)serverURL HTTPBody:(NSData *)HTTPBody {
    ...
    [request setValue:@"SendataAnalytics iOS SDK" forHTTPHeaderField:@"User-Agent"];
    if (SAModuleManager.sharedInstance.debugMode ==   SendataAnalyticsDebugOnly) {
