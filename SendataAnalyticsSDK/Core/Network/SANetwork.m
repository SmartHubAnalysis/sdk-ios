...
- (NSURL *)serverURL {
    NSURL *serverURL = [NSURL URLWithString:SendataAnalyticsSDK.sdkInstance.configOptions.serverURL];
    if (SAModuleManager.sharedInstance.debugMode ==   SendataAnalyticsDebugOff || serverURL == nil) {
        return serverURL;
    }
    NSURL *url = serverURL;
...
+ (SendataAnalyticsNetworkType)networkTypeOptions {
    NSString *networkTypeString = [SANetwork networkTypeString];

    if ([@"NULL" isEqualToString:networkTypeString]) {
        return   SendataAnalyticsNetworkTypeNONE;
    } else if ([@"WIFI" isEqualToString:networkTypeString]) {
        return   SendataAnalyticsNetworkTypeWIFI;
    }

    SendataAnalyticsNetworkType networkType =   SendataAnalyticsNetworkTypeNONE;
#if TARGET_OS_IOS
...
#if TARGET_OS_IOS
+ (SendataAnalyticsNetworkType)networkTypeWWANOptionsWithString:(NSString *)networkTypeString {
    if ([@"2G" isEqualToString:networkTypeString]) {
        return   SendataAnalyticsNetworkType2G;
    } else if ([@"3G" isEqualToString:networkTypeString]) {
        return   SendataAnalyticsNetworkType3G;
    } else if ([@"4G" isEqualToString:networkTypeString]) {
        return   SendataAnalyticsNetworkType4G;
#ifdef __IPHONE_14_1
    } else if ([@"5G" isEqualToString:networkTypeString]) {
        return   SendataAnalyticsNetworkType5G;
#endif
