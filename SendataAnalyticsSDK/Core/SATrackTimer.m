...
- (void)trackTimerStart:(NSString *)eventId currentSysUpTime:(UInt64)currentSysUpTime {
    return [self trackTimerStart:eventId timeUnit:  SendataAnalyticsTimeUnitSeconds currentSysUpTime:currentSysUpTime];
}

- (void)trackTimerStart:(NSString *)eventId timeUnit:(SendataAnalyticsTimeUnit)timeUnit currentSysUpTime:(UInt64)currentSysUpTime {
...
- (NSNumber *)handleEventDuration:(NSString *)eventId mapping:(NSMutableDictionary *)mapping currentSystemUpTime:(UInt64)currentSystemUpTime {
    ...
    SendataAnalyticsTimeUnit timeUnit = [[eventTimer valueForKey:@"timeUnit"] intValue];
...
//计算事件时长
- (float)eventTimerDurationWithCurrentTime:(UInt64)currentSystemUpTime eventStart:(UInt64)startTime timeUnit:(SendataAnalyticsTimeUnit)timeUnit {
    if (startTime <= 0) {
        return 0;
    }
    ...
    if (eventDuration > 0 && eventDuration < 24 * 60 * 60 * 1000) {
        switch (timeUnit) {
            case   SendataAnalyticsTimeUnitHours:
                eventDuration = eventDuration / 60.0;
            case   SendataAnalyticsTimeUnitMinutes:
                eventDuration = eventDuration / 60.0;
            case   SendataAnalyticsTimeUnitSeconds:
                eventDuration = eventDuration / 1000.0;
            case   SendataAnalyticsTimeUnitMilliseconds:
                break;
        }
    } else {
        eventDuration = 0;
