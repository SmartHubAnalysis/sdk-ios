...
#define SAPropertyError(errorCode, fromat, ...) \
    [NSError errorWithDomain:@"SendataAnalyticsErrorDomain" \
                        code:errorCode \
                    userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:fromat,##__VA_ARGS__]}] \

@protocol SAPropertyKeyProtocol <NSObject>

- (void)sendata_isValidPropertyKeyWithError:(NSError **)error;

@end

@protocol SAPropertyValueProtocol <NSObject>

- (id _Nullable)sendata_propertyValueWithKey:(NSString *)key error:(NSError **)error;

@end

@protocol SAEventPropertyValidatorProtocol <NSObject>

- (id _Nullable)sendata_validKey:(NSString *)key value:(id)value error:(NSError **)error;
