...
- (void)sendata_isValidPropertyKeyWithError:(NSError *__autoreleasing  _Nullable *)error {
    if (![SAValidator isValidKey: self]) {
        *error = SAPropertyError(10001, @"property name[%@] is not valid", self);
    }
}

- (id)sendata_propertyValueWithKey:(NSString *)key error:(NSError *__autoreleasing  _Nullable *)error {
...
- (id)sendata_propertyValueWithKey:(NSString *)key error:(NSError *__autoreleasing  _Nullable *)error {
    ...
    for (id element in self) {
        ...
        id sendataValue = [(id <SAPropertyValueProtocol>)element sendata_propertyValueWithKey:key error:error];
        [result addObject:sendataValue];
    }
    return [result copy];
}
...
- (id)sendata_validKey:(NSString *)key value:(id)value error:(NSError *__autoreleasing  _Nullable *)error {
    if (![key conformsToProtocol:@protocol(SAPropertyKeyProtocol)]) {
        *error = SAPropertyError(10004, @"Property Key should by %@", [key class]);
        return nil;
    }

    [(id <SAPropertyKeyProtocol>)key sendata_isValidPropertyKeyWithError:error];
    if (*error) {
        return nil;
    }
    ...
    // value 转换
    return [(id <SAPropertyValueProtocol>)value sendata_propertyValueWithKey:key error:error];
}
...
+ (NSMutableDictionary *)validProperties:(NSDictionary *)properties validator:(id<SAEventPropertyValidatorProtocol>)validator error:(NSError **)error {
    ...
    for (id key in properties) {
        id value = [validator sendata_validKey:key value:properties[key] error:error];
        if (*error) {
            return nil;
        }
