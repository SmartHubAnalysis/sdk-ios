...
- (NSString *)javaScriptSource {
    ...
    if (self.configOptions.serverURL) {
        NSString *initSource = @"window.Sendata_iOS_JS_Bridge = {};";
        NSString *setServerURLSource = [NSString stringWithFormat:@"window.Sendata_iOS_JS_Bridge.sendata_app_server_url = '%@';", self.configOptions.serverURL];
...
- (void)swizzleWebViewMethod {
    ...
    dispatch_once(&onceTokenWebView, ^{
        ...
        [WKWebView sa_swizzleMethod:@selector(loadRequest:)
                         withMethod:@selector(sendata_loadRequest:)
                              error:&error];

        [WKWebView sa_swizzleMethod:@selector(loadHTMLString:baseURL:)
                         withMethod:@selector(sendata_loadHTMLString:baseURL:)
                              error:&error];

        if (@available(iOS 9.0, *)) {
            [WKWebView sa_swizzleMethod:@selector(loadFileURL:allowingReadAccessToURL:)
                             withMethod:@selector(sendata_loadFileURL:allowingReadAccessToURL:)
                                  error:&error];

            [WKWebView sa_swizzleMethod:@selector(loadData:MIMEType:characterEncodingName:baseURL:)
                             withMethod:@selector(sendata_loadData:MIMEType:characterEncodingName:baseURL:)
                                  error:&error];
        }
...
- (void)addScriptMessageHandlerWithWebView:(WKWebView *)webView {
    ...
    @try {
        ...
        [userScripts enumerateObjectsUsingBlock:^(WKUserScript *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            if ([obj.source containsString:@"sendata_app_server_url"] || [obj.source containsString:@"sendata_visualized_mode"]) {
        ...
        if (!isContainJavaScriptBridge) {
            ...
            // 通知其他模块，开启打通 H5
            if ([javaScriptSource containsString:@"sendata_app_server_url"]) {
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    @try {
        ...
        if ([callType isEqualToString:@"app_h5_track"]) {
            ...
            [[SendataAnalyticsSDK sharedInstance] trackFromH5WithEvent:trackMessageString];
