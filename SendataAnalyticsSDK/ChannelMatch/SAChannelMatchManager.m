...
NSString * const kSAChannelDebugFlagKey = @"com.sendata.channeldebug.flag";
...
- (void)trackAppInstallEvent:(NSString *)event properties:(NSDictionary *)properties {
    // 先发送 track
    SendataAnalyticsSDK *sdk = [SendataAnalyticsSDK sharedInstance];
...
- (void)trackChannelWithEventObject:(SABaseEventObject *)obj properties:(nullable NSDictionary *)propertyDict {
    if (self.configOptions.enableAutoAddChannelCallbackEvent) {
        return [SendataAnalyticsSDK.sharedInstance trackEventObject:obj properties:propertyDict];
    }
    ...
    [SendataAnalyticsSDK.sharedInstance trackEventObject:obj properties:properties];
}
...
- (BOOL)handleURL:(NSURL *)url {
    ...
    SANetwork *network = [SendataAnalyticsSDK sharedInstance].network;
...
- (void)uploadUserInfoIntoWhiteList:(NSDictionary *)qureyItems {
    ...
    params[@"distinct_id"] = [[SendataAnalyticsSDK sharedInstance] distinctId];
