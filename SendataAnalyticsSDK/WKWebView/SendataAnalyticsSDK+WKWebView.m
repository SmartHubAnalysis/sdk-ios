...
#if __has_include("SendataAnalyticsSDK+WebView.h")
#error This file cannot exist at the same time with `SendataAnalyticsSDK+WebView.h`. If you usen't `UIWebView`, please delete it.
#endif

#import "SendataAnalyticsSDK+WKWebView.h"
...
static NSString * const kSAJSGetAppInfoScheme = @"sendataanalytics://getAppInfo";
static NSString * const kSAJSTrackEventNativeScheme = @"sendataanalytics://trackEvent";

@interface SendataAnalyticsSDK (WKWebViewPrivate)
...
@implementation SendataAnalyticsSDK (WKWebView)
...
- (void)addWebViewUserAgentSendataFlag {
    [self addWebViewUserAgentSendataFlag:YES];
}

- (void)addWebViewUserAgentSendataFlag:(BOOL)enableVerify  {
    [self addWebViewUserAgentSendataFlag:enableVerify userAgent:nil];
}

- (void)addWebViewUserAgentSendataFlag:(BOOL)enableVerify userAgent:(nullable NSString *)userAgent {
    __weak typeof(self) weakSelf = self;
    void (^ changeUserAgent)(BOOL verify, NSString *oldUserAgent) = ^void (BOOL verify, NSString *oldUserAgent) {
        ...
        if ([oldUserAgent rangeOfString:@"sa-sdk-ios"].location == NSNotFound) {
            strongSelf.addWebViewUserAgent = verify ? [NSString stringWithFormat:@" /sa-sdk-ios/sendata-verify/%@?%@ ", strongSelf.network.host, strongSelf.network.project] : @" /sa-sdk-ios";
...
- (BOOL)showUpWebView:(id)webView WithRequest:(NSURLRequest *)request andProperties:(NSDictionary *)propertyDict enableVerify:(BOOL)enableVerify {
    ...
    @try {
        ...
        NSString *js = [NSString stringWithFormat:@"sendata_app_js_bridge_call_js('%@')", jsonString];
