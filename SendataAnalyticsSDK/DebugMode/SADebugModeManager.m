...
- (void)handleDebugMode:(SendataAnalyticsDebugMode)mode {
    _debugMode = mode;

    if (_debugMode ==   SendataAnalyticsDebugOff) {
        return;
    }

    // 打开debug模式，弹出提示
    NSString *alertMessage = nil;
    if (_debugMode ==   SendataAnalyticsDebugOnly) {
        alertMessage = @"现在您打开了'DEBUG_ONLY'模式，此模式下只校验数据但不导入数据，数据出错时会以提示框的方式提示开发者，请上线前一定关闭。";
    } else if (_debugMode ==   SendataAnalyticsDebugAndTrack) {
...
- (void)showDebugModeAlertWithParams:(NSDictionary<NSString *, NSString *> *)params {
    dispatch_async(dispatch_get_main_queue(), ^{
        dispatch_block_t alterViewBlock = ^{
            ...
            if (self.debugMode ==   SendataAnalyticsDebugAndTrack) {
                alterViewMessage = @"开启调试模式，校验数据，并将数据导入神策分析中；\n关闭 App 进程后，将自动关闭调试模式。";
            } else if (self.debugMode ==   SendataAnalyticsDebugOnly) {
        ...
        void(^handler)(SendataAnalyticsDebugMode) = ^(SendataAnalyticsDebugMode debugMode) {
            self.debugMode = debugMode;
            alterViewBlock();
            [self debugModeCallbackWithDistinctId:[SendataAnalyticsSDK sharedInstance].distinctId params:params];
        };
        [alertController addActionWithTitle:@"开启调试模式（导入数据）" style:SAAlertActionStyleDefault handler:^(SAAlertAction * _Nonnull action) {
            handler(  SendataAnalyticsDebugAndTrack);
        }];
        [alertController addActionWithTitle:@"开启调试模式（不导入数据）" style:SAAlertActionStyleDefault handler:^(SAAlertAction * _Nonnull action) {
            handler(  SendataAnalyticsDebugOnly);
        }];
...
- (NSString *)debugModeToString:(SendataAnalyticsDebugMode)debugMode {
    NSString *modeStr = nil;
    switch (debugMode) {
        case   SendataAnalyticsDebugOff:
            modeStr = @"DebugOff";
            break;
        case   SendataAnalyticsDebugAndTrack:
            modeStr = @"DebugAndTrack";
            break;
        case   SendataAnalyticsDebugOnly:
            modeStr = @"DebugOnly";
            break;
        default:
            modeStr = @"Unknown";
...
- (void)showDebugModeWarning:(NSString *)message withNoMoreButton:(BOOL)showNoMore {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.debugMode ==   SendataAnalyticsDebugOff) {
            return;
        }
...
- (NSURL *)serverURL {
    return [SendataAnalyticsSDK sharedInstance].network.serverURL;
}
